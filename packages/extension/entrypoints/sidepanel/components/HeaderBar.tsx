...
export function HeaderBar({
  ...
  return (
    <header className="flex h-10 flex-none items-center gap-2 border-b border-ip-border-subtle bg-ip-bg-secondary/50 px-3 backdrop-blur-sm">
      ...
      <ConnectionChip variant={variant} canReconnect={canReconnect} onClick={onReconnect} />
    </header>
  );
}
...
function InspectToggle({ inspecting, disabled, onClick }: InspectToggleProps) {
  return (
    <button
      ...
      className={[
        "inline-flex h-7 items-center gap-1.5 rounded-ip-sm px-2.5 text-[11px] font-medium transition-all duration-150",
        inspecting
          ? "animate-glow-pulse bg-ip-error-muted text-ip-error hover:brightness-110"
          : disabled
    ...
    >
      {inspecting ? <StopIcon size={11} /> : <CrosshairIcon size={12} />}
      <span>{inspecting ? "Stop" : "Inspect"}</span>
    </button>
  );
}
...
function ConnectionChip({ variant, canReconnect, onClick }: ConnectionChipProps) {
  const dot =
    variant === "connected"
      ? "bg-ip-success"
